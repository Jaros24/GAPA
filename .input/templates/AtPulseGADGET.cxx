...
const Double_t rtTwo = TMath::Sqrt(2); // Square root of 2 as a constant


double erfApproxWinitzki(double x) {
   // APPROXIMATION FOR ERF FUNCTION, FASTER DROP IN REPLACEMENT FOR TMath Erf FUNCTION
    const double a = 0.147;
    const double factor = (4.0 / M_PI) + a * x * x;
    const double denominator = 1.0 + a * x * x;
    const double exponent = -x * x * factor / denominator;

    if (x >= 0) {
        return sqrt(1.0 - exp(exponent));
    } else {
        return -sqrt(1.0 - exp(exponent));
    }
}
...
Double_t AtPulseGADGET::ChargeDispersion(Double_t G, Double_t time, Double_t x0, Double_t y0, Double_t xi, Double_t yi)
{
    ...
    Double_t halfW = W / 2; // Precompute half of W
    Double_t rtTwoSigma = rtTwo * Sigma; // Precompute rtTwo * Sigma

    Double_t erfTermX1 = TMath::Erf((xi + halfW - x0) / rtTwoSigma);
    Double_t erfTermX2 = TMath::Erf((xi - halfW - x0) / rtTwoSigma);
    Double_t erfTermY1 = TMath::Erf((yi + halfW - y0) / rtTwoSigma);
    Double_t erfTermY2 = TMath::Erf((yi - halfW - y0) / rtTwoSigma);

    Double_t Charge = G / 4 * (erfTermX1 - erfTermX2) * (erfTermY1 - erfTermY2);
    return Charge;
};
